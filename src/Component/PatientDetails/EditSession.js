...
import e from '../../assets/images/editing 1.svg'
...
const EditSession = ({ data }) => {
    ...
    const initialState = {
        ...
        price: `${data.price}`,
        number: `${data.number_of_sessions}`,
        status: `${data.status}`,
        total: `${data.total}`,
        paid: `${data.paid}`
    };
    ...
    function finalize(e) {
        ...
        axios
            .post(`http://92.205.235.108:8000/clinic/api/sessions/${patientId.id}`, reqData, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
        ...
        <>
            ...
            <Modal show={show} onHide={handleClose} size='lg'>
                <Modal.Header closeButton>
                    <Modal.Title className={`${styles.label}`}>Edit Session</Modal.Title>
                </Modal.Header>
                <Modal.Body>
